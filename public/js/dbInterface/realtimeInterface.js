...
async function updateOnDb(path, value, id) {
    await update(ref(db, path + "/" + id), value);
}

export async function updatePazienteOnRealtimeDB(paziente) {
    if (paziente.cf_paziente == undefined)
        console.error("CF paziente obbligatorio");
    else await updateOnDb("/pazienti", paziente, paziente.cf_paziente);
}

export async function updateMedicoOnRealtimeDB(medico) {
    if (medico.cf_medico == undefined) console.error("CF medico obbligatorio");
    else await updateOnDb("/medici", medico, medico.cf_medico);
}

export async function updateCartellaOnRealtimeDB(cf_paziente, cartella) {
    await updateOnDb("/cartelle/" + cf_paziente, cartella);
}

export async function updateRefertoOnRealtimeDB(referto, id_cartella, cf_paziente, tipo_referto) {
    await update(`/cartelle/${cf_paziente}/${id_cartella}/referti/${tipo_referto}`, referto);
}
