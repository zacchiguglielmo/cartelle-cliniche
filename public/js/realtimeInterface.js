...
// CREATE
export function addOnDb(path, value) {
...
export function addPaziente(paziente) {
    if (paziente.cf_paziente == undefined)
        ...
        addOnDb('/pazienti', paziente, paziente.cf_paziente);
}

export function addMedico(medico) {
    if (medico.cf_medico == undefined)
        console.error("CF medico obbligatorio");
    else
        addOnDb('/medico', medico, medico.cf_medico);
};
